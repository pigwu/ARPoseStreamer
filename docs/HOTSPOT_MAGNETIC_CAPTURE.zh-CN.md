...
磁传感器在试验阶段也是可选输入：没有连接板卡时，ARKit 预览、位姿/视频录制、电脑端位姿传输、历史记录和补传都必须正常工作。此时 APM1 包的 `magnetic_count` 为 0，APP 只有在收到首个有效磁数据后才创建 `magnetic.csv`。

## 默认端口

| 方向 | 端口 | 用途 |
|---|---:|---|
| 板卡到手机 | UDP 5557 | 96 字节 ASKN 磁数据 |
| 手机到电脑 | UDP 5558 | APM1 位姿和磁数据组合包 |
...
4. 板卡保存热点 SSID 和密码，启动后自动重连。
5. 板卡从 DHCP 获取 Gateway 地址，并向 `Gateway:5557` 发送 ASKN 数据。不要把 `172.20.10.1` 写死在固件中。
6. 打开 APP。默认自动启动磁数据监听；设置页可以查看收包率、丢包、板卡地址和 5 个芯片的实时数值。
...
电脑每两秒向手机 UDP 5559 发送一次 `PC_HELLO`。APP 从数据包来源自动识别电脑地址，并把 APM1 组合数据发到电脑 UDP 5558。电脑断开时，APP 只停止实时转发，本地录制不会中断。
...
在 APP 中设置电脑 IP，再选择 `Past Records -> Upload Data`，即可上传 `pose.csv`、`magnetic.csv` 和 manifest；视频使用单独的 `Upload Video`。
