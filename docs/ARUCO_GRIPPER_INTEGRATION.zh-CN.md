...
`Live Monitor` 中的相机选择框可以查看 `1× Main (ARKit)` 或 `0.5× Ultra-wide (ArUco)`，仅改变电脑端显示，不影响两路同时接收和记录。实验上传完成后，同一数据文件夹内包含 `video.mp4` 与 `ultrawide_video.mp4`；`Experiment Replay` 中可用同样的选择框回放两路录像。

## 快捷两点标定

1. 让 0.5× 画面同时看到 ID 0、1。如果显示 `marker_depth_out_of_range`，打开 `ArUco Gripper`，点击“使用当前深度并应用”。程序会用当前两个深度的中点作为标称深度，并自动留出至少 `±20 mm` 余量。
2. 用卡尺填写最小实际开口；夹持面完全接触时可填 `0 mm`。
3. 用卡尺填写最大实际开口。
4. 夹爪保持在最小开口约 1 秒，点击“记录当前最小点”。
5. 夹爪保持在最大开口约 1 秒，点击“记录当前最大点”。
6. 点击“完成：保存并应用标定”。

快捷记录会取最近约 1 秒稳定帧的中位数；如果仍在移动或有效帧不足，UI 会提示保持不动后重试。原来的至少 5 次连续开合稳健采集仍保留在“可选：显示连续开合稳健采集”中，不再是必需步骤。
...
```json
{
  ...
  "nominal_marker_depth_m": 0.1101,
  "marker_depth_tolerance_m": 0.0200,
  "distance_scale": 1.0,
...
实验上传完成后，电脑会在后台逐帧处理 `ultrawide_video.mp4`，并在同一实验文件夹生成：

- `aruco_gripper.csv`：每帧时间、检测状态、原始 X 宽度、校准距离、离线稳定距离和标记深度。
- `aruco_gripper_state.json`：处理状态、有效帧率、内参来源和使用的内参。

离线稳定距离使用居中的 5 帧中值滤波，并只对最多连续 2 个缺失帧做线性插值，因此没有实时滤波的因果延迟，也不会填补长时间看不到标记的区间。`Experiment Replay` 页签会在时间游标处显示原始、校准和离线稳定距离，并在 `Gripper` 图表中同时绘制逐帧值和稳定值。需要使用新标定重新计算时，点击 `Process / Reprocess 0.5× Gripper Distance`。

新的录像优先使用电脑从 APV2 实时流保存的真实超广角内参。旧 MP4 没有内参元数据时，程序会根据 16 mm 标记、当前标称深度和视频中的标记像素尺寸估计内参，并在状态文件和 UI 中标记为估计值。
